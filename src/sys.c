...
static void sys_update(void)
{
	...
	/* if (video_buf) gsp_image(cpu_ptr(video_buf)); */
	if (!sys_fast) sleep_frame();
	app_update();
	if (sys_prenmi > 0)
	{
		if (--sys_prenmi == 0) longjmp(sys_jmp, TH_RESET);
	}
	else if (sys_reset)
	{
		sys_reset  = FALSE;
...
#error add asm here
	...
	os_thread = NULL;
	while (os_thread_list) th_destroy(os_thread_list);
	while (os_timer_list) tm_destroy(os_timer_list);
	start();
}
