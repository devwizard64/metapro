static void asp_a_adpcm(u32 w0, u32 w1)
{
#ifdef ASP_MAIN1
    uint  flag    = w0 >> 16;
#endif
...
#endif
#ifdef ASP_NAUDIO
    uint  dmemin  = asp_sample + (w0 >> 12 & 0x0F);
    s16  *dmemout = asp_s16(asp_sample + (w0 >> 0 & 0x0FFF));
#endif
    s16  *adpcm   = asp_s16(asp_adpcm);
    int   count   = asp_count;
    if (flag & A_INIT)
    {
        memset(dmemout, 0x00, 0x20);
    }
    else
    {
